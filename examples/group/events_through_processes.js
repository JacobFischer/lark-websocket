...
global.syncEvents.push('global_event');
global.on('global_event', function(message){
    global.all().connectedWithMe().send("Server ["+ os.hostname() + " pid : " +process.pid+ "] Global Event : " + message);
});
global.on('simple_event', function(message){
...
wsApp.on('message',function(message){
    ...
    global.all().send('#ALL#');
    global.emit('global_event', '(' + message + ')');
    global.emit('simple_event', '{' + message + '}');
    return this;
});
