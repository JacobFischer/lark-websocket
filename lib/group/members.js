    ...
    Object.keys(Members.iterator).forEach(function(key){
        Members.iterator[key].forEach(function(func){
            Group.prototype[func] = function(){
                ...
                if(typeof members[func] != 'function') return this.emit("Function " + func + " not found");
...
Members.upgradeObj = function(group){
    group.on('sync', function(steps){
        group.getMembers().sync(steps);
    });
};
...
Members.prototype.run = function(noSync){
    if(!noSync && this.group.isSync()){
        this.group.emit('sync', this.steps);
        /**
         * In sync model, they don't record the iterator
...
Members.prototype.sync = function(steps){
    this.steps = steps;
    this.run(true);
};
